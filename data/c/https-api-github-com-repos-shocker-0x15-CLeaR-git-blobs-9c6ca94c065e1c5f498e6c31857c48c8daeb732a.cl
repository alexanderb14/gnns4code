...
uint O(global uint* a);
inline float P(global uint* a);
inline uint Q(float a, uint b);
void R(float a, float b, float* c, float* d);
vector3 S(float a, float b);
void T(float a, float b, float* c, float* d);
...
inline float P(global uint* a) {
  return __builtin_astype((O(a) >> 9) | 0x3f800000, float) - 1.0f;
}
