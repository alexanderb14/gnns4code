...
__kernel void B(__global float* a, __global float* b, __global float* c, __global float* d, __global int* e, int f, int g, float h, float i, float j, float k, float l, float m, __local float* n, __local float* o, __local float* p, int q, int r) {
  float s, t, u;
  ...
  int z = get_local_size(1);
  int aa = get_local_size(0);

  n[x * aa + y] = c[v * g + w];

  barrier(1);

  if ((v > 0 && v < f - 1) && (w > 0 && w < g - 1)) {
    if ((e[v * g + w] & 0x0010) && (e[v * g + w] < 0x1000)) {
      if (x < r && y < q) {
        s = (n[(x + 1) * aa + y] - 2.0 * n[x * aa + y] + n[(x - 1) * aa + y]) * (1. / i / i) + (n[x * aa + y + 1] - 2.0 * n[x * aa + y] + n[x * aa + y - 1]) * (1. / j / j);
        t = ((a[v * g + w] * 0.5 * (n[x * aa + y] + n[(x + 1) * aa + y]) - a[(v - 1) * g + w] * 0.5 * (n[(x - 1) * aa + y] + n[x * aa + y])) + k * (__clc_fabs(a[v * g + w]) * 0.5 * (n[x * aa + y] - n[(x + 1) * aa + y]) - __clc_fabs(a[(v - 1) * g + w]) * 0.5 * (n[(x - 1) * aa + y] - n[x * aa + y]))) / i;
        u = ((b[v * g + w] * 0.5 * (n[x * aa + y] + n[x * aa + y + 1]) - b[v * g + w - 1] * 0.5 * (n[x * aa + y - 1] + n[x * aa + y])) + k * (__clc_fabs(b[v * g + w]) * 0.5 * (n[x * aa + y] - n[x * aa + y + 1]) - __clc_fabs(b[v * g + w - 1]) * 0.5 * (n[x * aa + y - 1] - n[x * aa + y]))) / j;
...
__kernel void D(__global float* a, __global float* b, __global float* c, __global int* d, int e, int f, float g, float h, float i, __local float* j, __local float* k) {
  e = e + 2;
  f = f + 2;
  ...
  int p = get_local_size(1);
  int q = get_local_size(0);

  if ((l > 0 && l < e - 1) && (m > 0 && m < f - 1)) {
    j[n * q + o] = a[l * f + m] - a[(l - 1) * f + m];
    k[n * q + o] = b[l * f + m] - b[l * f + m - 1];
  } else {
    j[n * q + o] = 0;
    k[n * q + o] = 0;
  }
  ...
  if ((l > 0 && l < e - 1) && (m > 0 && m < f - 1)) {
    if ((d[l * f + m] & 0x0010) && (d[l * f + m] < 0x0100)) {
      c[l * f + m] = (j[n * q + o] / h + k[n * q + o] / i) / g;
    }
  }
}
