__kernel void A(__global double4* a, __global double4* b, __global double4* c, uint d, uint e) {
  ...
  for (int k = 0; k < d; k = k + 4) {
    double4 l = a[k / 4 + (f.y << 2) * (d / 4)];
    double4 m = a[k / 4 + ((f.y << 2) + 1) * (d / 4)];
    double4 n = a[k / 4 + ((f.y << 2) + 2) * (d / 4)];
    double4 o = a[k / 4 + ((f.y << 2) + 3) * (d / 4)];

    double4 p = b[f.x + k * e];
    double4 q = b[f.x + (k + 1) * e];
    double4 r = b[f.x + (k + 2) * e];
    double4 s = b[f.x + (k + 3) * e];
...
__kernel void B(__global double4* a, __global double4* b, __global double4* c, int d, __local double4* e) {
  ...
  for (int m = 0; m < (l / get_local_size(0)); m++) {
    ...
    for (int p = 0; p < get_local_size(0) * 4; p = p + 4) {
      double4 q = e[(p >> 2) + get_local_id(1) * 4 * get_local_size(0)];
      double4 r = e[(p >> 2) + (get_local_id(1) * 4 + 1) * get_local_size(0)];
      double4 s = e[(p >> 2) + (get_local_id(1) * 4 + 2) * get_local_size(0)];
      double4 t = e[(p >> 2) + (get_local_id(1) * 4 + 3) * get_local_size(0)];

      double4 u = b[o + p * get_global_size(0)];
      double4 v = b[o + (p + 1) * get_global_size(0)];
      double4 w = b[o + (p + 2) * get_global_size(0)];
      double4 x = b[o + (p + 3) * get_global_size(0)];
