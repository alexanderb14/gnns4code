...
__kernel void V(__global uint* a) {
  mwc64xvec2_state_t b = {(uint2)(0, 0), (uint2)(1, 2)}, c;
...
__kernel void W(uint a, __global uint* b) {
  ulong c = a / (2 * get_global_size(0));

  mwc64xvec2_state_t d;
  L(&d, 0, c);

  for (uint e = 0; e < c; e++) {
    uint2 f = M(&d);
    b[get_global_id(0) * 2 * c + 0 * c + e] = f.s0;
    b[get_global_id(0) * 2 * c + 1 * c + e] = f.s1;
  }
}

__kernel void X(ulong a, ulong b, __global ulong* c) {
  mwc64xvec2_state_t d;
  ulong e = a / (2 * get_global_size(0));
  L(&d, b, 2 * e);
  uint f = 0;
  for (uint g = 0; g < e; g++) {
    ulong2 h = convert_ulong2(M(&d));
    ulong2 i = convert_ulong2(M(&d));
    ulong2 j = h * h;
    ulong2 k = i * i;
    long2 l = -(j + k >= j);
    f += l.x + l.y;
