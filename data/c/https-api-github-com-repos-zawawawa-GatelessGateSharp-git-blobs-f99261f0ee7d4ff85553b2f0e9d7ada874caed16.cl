...
__attribute__((reqd_work_group_size(128, 1, 1))) __kernel void B(__global const uchar* restrict a, __global uint* restrict b, const uint c, const ulong d, __constant uint8* e) {
  uint f = get_global_id(0) + c;
  uint16 g;
  uint8 h;
  g = Gb;
  g.s0 = (__builtin_astype(__builtin_astype(((__global const uint*)a)[48], uchar4).wzyx, uint));
  g.s1 = (__builtin_astype(__builtin_astype(f, uchar4).wzyx, uint));
  h = A(g, *e);
  g.lo = h;
  g.hi = Gc;
  h = A(g, Ga);

  if (__builtin_astype(h.s10, ulong) <= d) {
    b[atomic_add(b + 0xFF, 1)] = f;
  }
}