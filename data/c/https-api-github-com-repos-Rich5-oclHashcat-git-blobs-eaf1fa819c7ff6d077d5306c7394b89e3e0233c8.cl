...
void CB(__local RC4_KEY* a, u32 b[4], u32 c[4], u32 d[4], u32 e[4], const u32 f, __global pw_t* g, __global kernel_rule_t* h, __global comb_t* i, __global bf_t* j, __global void* k, __global void* l, __global u32* m, __global u32* n, __global u32* o, __global u32* p, __global u32* q, __global u32* r, __global u32* s, __global u32* t, __global plain_t* u, __global digest_t* v, __global u32* w, __global salt_t* x, __global oldoffice34_t* y, __global u32* z, __global u32* aa, const u32 ab,
  ...
  const u32 ao = y[ae].version;

  u32 ap[4];

  ap[0] = y[ae].encryptedVerifier[0];
  ap[1] = y[ae].encryptedVerifier[1];
  ap[2] = y[ae].encryptedVerifier[2];
  ap[3] = y[ae].encryptedVerifier[3];

  u32 aq = b[0];

  for (u32 ar = 0; ar < ah; ar += 1) {
    const u32 as = BU(j, ar);
    ...
    { const u32 bc[4] = { ba0], ba1], ba2], ba3] };
      if (Z(bc, m, n, o, p, q, r, s, t, ab, ac, ad)) {
        int bd = X(bc, ai, &v[aj]);
        if (bd != -1) {
          const u32 be = aj + bd;
          if (atomic_add(&w[be], 1) == 0) {
            AA(u, z, ae, bd, be, ak, ar);
          }
        }
      }
    };
...
for (u32 as = 0; as < ah; as += 1) {
  const u32 at = BU(j, as);
  ...
  {
    if (((bb[0]) == aq[0]) && ((bb[1]) == aq[1]) && ((bb[2]) == aq[2]) && ((bb[3]) == aq[3])) {
      const u32 bd = aj + 0;
      if (atomic_add(&w[bd], 1) == 0) {
        AA(u, z, ae, 0, bd, ak, as);
      }
    }
  };
}
