...
__kernel void D(__global const int* a, __global const int* b, __global const int* c, __global uint* d, __global char* e) {
  __private int f = get_global_id(0);
  int g = a[f];
  int h = b[f];
  int i = *c;
  char j[32];
  ...
  for (int k = g; k < h; k++) {
    ...
    uint p[4];
    C(j, l, &p);
    if (p[0] == d[0] && p[1] == d[1] && p[2] == d[2] && p[3] == d[3]) {
      for (int k = 0; k < l; k++) {
        e[k] = j[k];
        e[k + 1] = 0x00;
      }
