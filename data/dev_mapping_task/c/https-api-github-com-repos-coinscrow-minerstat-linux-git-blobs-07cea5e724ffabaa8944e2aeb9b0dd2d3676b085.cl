...
void G(uint4 a[8], __global uint4* restrict b) {
  D(a);
  const uint c = 8;
  const uint d = (Ga[10] / 16 + (Ga[10] % 16 > 0));
  const uint e = 128;
  uint f = get_global_id(0) % e;

  for (uint g = 0; g < Ga[10] / 16; ++g) {
    for (uint h = 0; h < c; ++h)
      b[h + f * (c) + g * (e) * (c)] = a[h];
    for (uint i = 0; i < 16; ++i)
      F(a);
...

  {
    ...
    for (uint h = 0; h < c; ++h)
      b[h + f * (c) + g * (e) * (c)] = a[h];
    for (uint i = 0; i < Ga[10] % 16; ++i)
      F(a);
  }

  for (uint i = 0; i < Ga[10]; ++i) {
    uint4 j[8];
    uint k = a[7].x & (Ga[10] - 1);
    uint g = (k / 16);
    for (uint h = 0; h < c; ++h)
      j[h] = b[h + f * (c) + g * (e) * (c)];
 ...
 for (uint r=0; r<4; r++)
  {
    ...
    A(h + (r << 1), h + (r << 1) + 1, p, q, (uint4)(Gcc[84], 0U, 0U, 0U), (uint4)(0U, 0U, 0U, Gcc[88]));
  }
  G(h,c);
