...
float AH(const float3 a, const float3 b, const float c, const float d) {
      Microfacet e;
      float3 f = normalize(a + b);
      float g = D(f);
      e.D = AC(g, d);
      e.G = AG(a, b, f);
      e.F = Y(g, c);
...
float AI(const float3 a, const float3 b, const float c) {
      if (!L(a, b))
        return 0.f;
      ...
      float f = D(d);
      float g = ((c + 1.f) * __clc_pow(f, c)) / (2.f * 3.14 * 4.f * e);

      return e > 0.f ? g : 0.f;
}

float3 AJ(const float3 a, const float3 b, float c, const float2 d) {
      float e = __clc_pow(d.x, 1.f / (c + 1.f));
      float f = sqrt(max(0.f, 1.f - e * e));
      float g = d.y * 2.f * 3.14;
      float3 h = O(f, e, g);
      if (!M(a, h, b))
        h = -h;
