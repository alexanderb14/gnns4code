...
void DZ(const u32 a[4], const u32 b[4], const u32 c[4], const u32 d[4], u32 e[5]) {
  u32 f = e[0];
  u32 g = e[1];
  u32 h = e[2];
  u32 i = e[3];
...
__kernel void EA(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global void* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  for (u32 an = 0; an < ab; an += 1) {
    ...
    for (bc = 0; bc < ba - 4; bc += 4) {
      const u32 be = U((Ga)[((bb + bc) / 4) + 1], (Ga)[((bb + bc) / 4) + 0], (bb + bc));

      {
        const u32 bf = ((bd + bc) & 3) * 8;
        const u64 bg = (u64)(be) << bf;
        (ay)[((bd + bc) / 4) + 0] |= bg;
        (ay)[((bd + bc) / 4) + 1] = bg >> 32;
      };
    }

    const u32 bh = 0xffffffff >> (((bc - ba) & 3) * 8);

    const u32 be = U((Ga)[((bb + bc) / 4) + 1], (Ga)[((bb + bc) / 4) + 0], (bb + bc)) & bh;
    ...
    for (bc = 0; bc < am + 1; bc += 4) {
      const u32 be = al[bc / 4];
    ...
    for (bi = bd, bj = 0; bi >= 56; bi -= 64, bj += 16) {
      DY(&ay[bj]);

      DZ(&ay[bj + 0], &ay[bj + 4], &ay[bj + 8], &ay[bj + 12], az);
    }
    ...
    {
      const u32 bk[4] = {az az az az};
      if (Z(bk, g, h, i, j, k, l, m, n, v, w, x)) {
        int bl = X(bk, ac, &p[ad]);
        if (bl != -1) {
          const u32 bm = ad + bl;
          if (atomic_add(&q[bm], 1) == 0) {
            AA(o, t, y, bl, bm, ah, an);
          }
        }
      }
    };
...
for (u32 ao = 0; ao < ab; ao += 1) {
  ...
  for (bd = 0; bd < bb - 4; bd += 4) {
    const u32 bf = U((Ga)[((bc + bd) / 4) + 1], (Ga)[((bc + bd) / 4) + 0], (bc + bd));

    {
      const u32 bg = ((be + bd) & 3) * 8;
      const u64 bh = (u64)(bf) << bg;
      (az)[((be + bd) / 4) + 0] |= bh;
      (az)[((be + bd) / 4) + 1] = bh >> 32;
    };
  }

  const u32 bi = 0xffffffff >> (((bd - bb) & 3) * 8);

  const u32 bf = U((Ga)[((bc + bd) / 4) + 1], (Ga)[((bc + bd) / 4) + 0], (bc + bd)) & bi;
  ...
  for (bd = 0; bd < am + 1; bd += 4) {
    const u32 bf = al[bd / 4];
  ...
  for (bj = be, bk = 0; bj >= 56; bj -= 64, bk += 16) {
    DY(&az[bk]);

    DZ(&az[bk + 0], &az[bk + 4], &az[bk + 8], &az[bk + 12], ba);
  }
  ...
  {
    if (((ba[3]) == an[0]) && ((ba[4]) == an[1]) && ((ba[2]) == an[2]) && ((ba[1]) == an[3])) {
      const u32 bl = ad + 0;
      if (atomic_add(&q[bl], 1) == 0) {
        AA(o, t, y, 0, bl, ah, ao);
      }
    }
  };
