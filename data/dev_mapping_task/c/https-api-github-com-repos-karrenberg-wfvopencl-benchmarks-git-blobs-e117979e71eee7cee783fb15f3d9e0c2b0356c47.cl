...
uchar4 E(__local uchar4* a, __private uchar4* b, unsigned int c, unsigned int d, unsigned int e) {
  unsigned int f = 4;
  ...
  for (unsigned int l = 1; l < 4; ++l) {
    g ^= D(a[l * e + k].x, b[(l + f - c) % f].x);
    h ^= D(a[l * e + k].y, b[(l + f - c) % f].x);
    i ^= D(a[l * e + k].z, b[(l + f - c) % f].x);
    j ^= D(a[l * e + k].w, b[(l + f - c) % f].x);
  }
