float A(float a) {
  return (atan(a) * (1.0f / 3.14159f)) + 0.5f;
}

__kernel void B(__global float* a, __global float* b, __constant int* c, int __constant* d, __constant int* e, float f, float g, float h, int i, float j) {
  ...
  while ((!y) && (ad < i)) {
    ...
    for (u = 0; u < s; u++) {
      ...
      if (v < q) {
        ...
        float ax = A((ap * -f) * ac);
        float ay = A((aq * f) * ac);
        float az = A((ar * (f - g)) * ac);
        float ba = A((as * (f + g)) * ac);
        float bb = A((at * (-f - g)) * ac);
        float bc = A((au * (-f + g)) * ac);
