...
__kernel void BZ(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global void* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  for (u32 an = 0; an < ab; an += 1) {
    ...
    ((be)[(bd) / 4] = (((be)[(bd) / 4] & ~(0xff << (((bd)&3) * 8))) | ((0x80) << (((bd)&3) * 8))));
    ...
    const u32 bj = BY(ay, ap, al, am, bf, bg, bh, bi, be);

    {
      const u32 bk = ((bj)&3) * 8;
      const u64 bl = (u64)(0x80) << bk;
      (be)[((bj) / 4) + 0] &= ~(0xff << ((bj & 3) * 8));
      (be)[((bj) / 4) + 0] |= bl;
      (be)[((bj) / 4) + 1] = bl >> 32;
    };
    ...
    {
      const u32 bm[4] = {bff, bgg, bhh, bii};
      if (Z(bm, g, h, i, j, k, l, m, n, v, w, x)) {
        int bn = X(bm, ac, &p[ad]);
        if (bn != -1) {
          const u32 bo = ad + bn;
          if (atomic_add(&q[bo], 1) == 0) {
            AA(o, t, y, bn, bo, ah, an);
          }
        }
      }
    };
...
for (u32 ao = 0; ao < ab; ao += 1) {
  ...
  ((bf)[(be) / 4] = (((bf)[(be) / 4] & ~(0xff << (((be)&3) * 8))) | ((0x80) << (((be)&3) * 8))));
  ...
  const u32 bk = BY(az, aq, al, am, bg, bh, bi, bj, bf);

  {
    const u32 bl = ((bk)&3) * 8;
    const u64 bm = (u64)(0x80) << bl;
    (bf)[((bk) / 4) + 0] &= ~(0xff << ((bk & 3) * 8));
    (bf)[((bk) / 4) + 0] |= bm;
    (bf)[((bk) / 4) + 1] = bm >> 32;
  };
  ...
  {
    if (((bg) == an[0]) && ((bh) == an[1]) && ((bi) == an[2]) && ((bj) == an[3])) {
      const u32 bn = ad + 0;
