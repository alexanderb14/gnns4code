...
__kernel void J(__global uchar* a, uint b, uint c, uint d, uint e, uint f, uint g, uint h, __global uchar* i, uint j, uint k, uint l, uint m, uint n, uint o, uint p, __global uchar* q, uint r, uint s, uint t) {
  ...
  const float2 x = vload2(0, (__global float*)w.ptr);

  const float4 y = (float4)x.s0;
  const float4 z = (float4)((x.s1 - x.s0) / 255.0f);

  const uchar4 aa = vload4(0, (__global uchar*)u.ptr);

  const float4 ab = convert_float4(aa) * z + y;

  vstore4(ab, 0, (__global float*)v.ptr);
}