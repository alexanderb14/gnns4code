...
void O(shabal_context_t* a, __global unsigned char* b, unsigned int c, unsigned int d) {
...
void P(shabal_context_t* a, unsigned char* b, unsigned int c) {
...
__kernel void Q(__global unsigned char* a, unsigned int b, unsigned long c, unsigned long d) {
  ...
  unsigned long f = d + e;
  unsigned int g = ((4096 * (2 * 32)) + 16) * e;

  M(a, g + (4096 * (2 * 32)), c);
  M(a, g + (4096 * (2 * 32)) + 8, f);
}

__kernel void R(__global unsigned char* a, unsigned int b, unsigned long c, unsigned int d, unsigned int e) {
  size_t f = get_global_id(0);
  if (f >= b) {
    return;
  }

  unsigned int g = ((4096 * (2 * 32)) + 16) * f;
  unsigned char h[32];
...
__kernel void S(__global unsigned char* a, unsigned int b) {
  size_t c = get_global_id(0);
  if (c >= b) {
    return;
  }

  unsigned int d = ((4096 * (2 * 32)) + 16) * c;
  unsigned char e[32];

  shabal_context_t f;
  N(&f);
  O(&f, a, d, ((4096 * (2 * 32)) + 16));
  P(&f, e, 0);

  barrier(1);

  unsigned int g = (4096 * (2 * 32));
