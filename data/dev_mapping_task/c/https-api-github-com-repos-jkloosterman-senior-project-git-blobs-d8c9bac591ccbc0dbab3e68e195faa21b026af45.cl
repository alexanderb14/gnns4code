...
__kernel void V(__global uint* a) {
  mwc64xvec4_state_t b = {(uint4)(0, 0, 0, 0), (uint4)(1, 2, 3, 4)}, c;
...
__kernel void W(uint a, __global uint* b) {
  ulong c = a / (4 * get_global_size(0));

  mwc64xvec4_state_t d;
  P(&d, 0, c);

  for (uint e = 0; e < c; e++) {
    uint4 f = Q(&d);
    b[get_global_id(0) * 4 * c + 0 * c + e] = f.s0;
    b[get_global_id(0) * 4 * c + 1 * c + e] = f.s1;
    b[get_global_id(0) * 4 * c + 2 * c + e] = f.s2;
    b[get_global_id(0) * 4 * c + 3 * c + e] = f.s3;
  }
}

__kernel void X(ulong a, ulong b, __global ulong* c) {
  mwc64xvec4_state_t d;
  ulong e = a / (4 * get_global_size(0));
  P(&d, b, 2 * e);
  uint f = 0;
  for (uint g = 0; g < e; g++) {
    ulong4 h = convert_ulong4(Q(&d));
    ulong4 i = convert_ulong4(Q(&d));
    ulong4 j = h * h;
    ulong4 k = i * i;
    long4 l = -(j + k >= j);
    f += l.x + l.y + l.z + l.w;
