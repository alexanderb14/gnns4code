__constant uchar Ga[255] = {};

__constant uchar Gb[256] = {};

uchar A(uchar a, uchar b) {
  uchar c = 0;
  uchar d;
  for (uchar e = 0; e < 8; e++) {
    if ((b & 1) == 1)
      c ^= a;
    d = (a & 0x80);
    a <<= 1;
    if (d == 0x80)
      a ^= 0x1b;
    b >>= 1;
  }
  return c;
}

void B(__global const uchar* a, uchar* b, uchar c) {
  const size_t d = c * 16;
  for (uint e = 0; e < 16; e++)
    b[e] ^= a[d + e];
}

void C(uchar* a) {
  for (uint b = 0; b < 16; b++)
    a[b] = Gb[a[b]];
}

void D(uchar* a) {
  uchar b[16];
  for (uint c = 0; c < 16; c++)
    b[c] = a[c];

  for (uint c = 0; c < 16; c++) {
    uint d = (c + ((c % 4) * 4)) % 16;
    a[c] = b[d];
  }
}

void E(uchar* a, uint b) {
  const uchar c = A(a[b + 0], 2), d = A(a[b + 1], 2), e = A(a[b + 2], 2), f = A(a[b + 3], 2);

  const uchar g = c ^ a[b + 3] ^ a[b + 2] ^ d ^ a[b + 1];
  const uchar h = d ^ a[b + 0] ^ a[b + 3] ^ e ^ a[b + 2];
  const uchar i = e ^ a[b + 1] ^ a[b + 0] ^ f ^ a[b + 3];
  const uchar j = f ^ a[b + 2] ^ a[b + 1] ^ c ^ a[b + 0];

  a[b + 0] = g;
  a[b + 1] = h;
  a[b + 2] = i;
  a[b + 3] = j;
}

void F(uchar* a) {
  E(a, 0);
  E(a, 4);
  E(a, 8);
  E(a, 12);
}

__kernel void G(__global const uchar* a, const uint b, __global const uchar* c, __global uchar* d, const uint e) {
  const size_t f = 16 * e;

  uchar g[16];
  for (uint h = 0; h < 16; h++) {
    g[h] = c[f + h];
  }

  B(a, g, 0);

  for (uint i = 1; i < b; i++) {
    C(g);
    D(g);
    F(g);
    B(a, g, i);
  }

  C(g);
  D(g);
  B(a, g, b);

  for (uint h = 0; h < 16; h++) {
    d[f + h] = g[h];
  }
}

__kernel void H(__global const uchar* a, const uint b, __global const uchar* c, __global uchar* d, const uint e) {
  const size_t f = get_global_id(0);
  if (f > e)
    return;

  const size_t g = 16 * f;

  uchar h[16];
  for (uint i = 0; i < 16; i++) {
    h[i] = c[g + i];
  }

  B(a, h, 0);

  for (uint j = 1; j < b; j++) {
    C(h);
    D(h);
    F(h);
    B(a, h, j);
  }

  C(h);
  D(h);
  B(a, h, b);

  for (uint i = 0; i < 16; i++) {
    d[g + i] = h[i];
  }
}