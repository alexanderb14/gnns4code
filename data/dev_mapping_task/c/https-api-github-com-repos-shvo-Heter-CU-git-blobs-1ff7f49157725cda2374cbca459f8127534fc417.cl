typedef float DATA_TYPE; __kernel __attribute__((reqd_work_group_size(1, 1, 1))) void A(__global DATA_TYPE* a, __global DATA_TYPE* b) {
  __local DATA_TYPE c[256 + 256 * 2] __attribute__((xcl_array_partition(cyclic, 4, 1)));
  ;
  __local DATA_TYPE d[256 + 256 * 2] __attribute__((xcl_array_partition(cyclic, 4, 1)));
  ;
  ...
  for (g = 1; g <= 256; ++g) {
    if (e == 0) {
      __attribute__((xcl_pipeline_loop)) for (f = 0; f < 256 + 256 - g; ++f) {
        if (f == 0) {
          d[f] = c[f];
        } else {
      ...
      __attribute__((xcl_pipeline_loop)) for (f = 0; f < 256 + 256 * 2 - g * 2; ++f) {
      ...
      __attribute__((xcl_pipeline_loop)) for (f = g; f < 256 + 256; ++f) {
        if (f == 256 + 256 - 1) {
          d[f] = c[f];
        } else {
          d[f] = 0.33333f * (c[f - 1] + c[f] + c[f + 1]);
