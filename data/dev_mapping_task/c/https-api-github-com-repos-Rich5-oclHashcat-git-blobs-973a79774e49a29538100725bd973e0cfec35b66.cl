...
void ED(const u32x a[16], const u32 b, u32x c[4], __local u32* d) {
  u32x e[4];
...
__kernel void EE(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global void* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  for (u32 ak = ah; ak < 256; ak += ai) {
    const u32 am = (ak >> 0) & 15;
    const u32 an = (ak >> 4) & 15;

    al[ak] = ((am < 10) ? '0' + am : 'A' - 10 + am) << 8 | ((an < 10) ? '0' + an : 'A' - 10 + an) << 0;
  }
  ...
  const u32 aq = a[ag].pw_len;

  const u32 ar = r[y].salt_buf[0];
  const u32 as = (r[y].salt_buf[1] & 0xff) | '(' << 8;
  ...
  for (u32 at = 0; at < ab; at += 1) {
    ...
    const u32x ay = BW(ao, ap, aq, b, at, au, av);
    ...
    const u32x bb = (u32x)(al[((ba[0] >> 0) & 255)]) << 0 | (u32x)(al[((ba[0] >> 8) & 255)]) << 16;
    const u32x bc = (u32x)(al[((ba[0] >> 16) & 255)]) << 0 | (u32x)(al[((ba[0] >> 24) & 255)]) << 16;
    const u32x bd = (u32x)(al[((ba[1] >> 0) & 255)]) << 0 | (u32x)(al[((ba[1] >> 8) & 255)]) << 16;
    const u32x be = (u32x)(al[((ba[1] >> 16) & 255)]) << 0 | (u32x)(al[((ba[1] >> 24) & 255)]) << 16;
    const u32x bf = (u32x)(al[((ba[2] >> 0) & 255)]) << 0 | (u32x)(al[((ba[2] >> 8) & 255)]) << 16;
    const u32x bg = (u32x)(al[((ba[2] >> 16) & 255)]) << 0 | (u32x)(al[((ba[2] >> 24) & 255)]) << 16;
    const u32x bh = (u32x)(al[((ba[3] >> 0) & 255)]) << 0 | (u32x)(al[((ba[3] >> 8) & 255)]) << 16;

    const u32 bi = 0x0e0e0e0e;
    ...
    u32x bj = ba[0] & 0xffffffff;
    u32x bk = ba[1] & 0xffffffff;
    u32x bl = ba[2] & 0x000000ff;
    u32x bm = ba[3] & 0x00000000;

    {
      const u32 bn[4] = {bjj, bkk, bll, bmm};
      if (Z(bn, g, h, i, j, k, l, m, n, v, w, x)) {
        int bo = X(bn, ac, &p[ad]);
        if (bo != -1) {
          const u32 bp = ad + bo;
          if (atomic_add(&q[bp], 1) == 0) {
            AA(o, t, y, bo, bp, ag, at);
          }
        }
      }
    };
...
for (u32 au = 0; au < ab; au += 1) {
  u32x av[4] = {0};
  u32x aw[4] = {0};
  u32x ax[4] = {0};
  u32x ay[4] = {0};

  const u32x az = BW(ao, ap, aq, b, au, av, aw);
  ...
  const u32x bc = (u32x)(al[((bb[0] >> 0) & 255)]) << 0 | (u32x)(al[((bb[0] >> 8) & 255)]) << 16;
  const u32x bd = (u32x)(al[((bb[0] >> 16) & 255)]) << 0 | (u32x)(al[((bb[0] >> 24) & 255)]) << 16;
  const u32x be = (u32x)(al[((bb[1] >> 0) & 255)]) << 0 | (u32x)(al[((bb[1] >> 8) & 255)]) << 16;
  const u32x bf = (u32x)(al[((bb[1] >> 16) & 255)]) << 0 | (u32x)(al[((bb[1] >> 24) & 255)]) << 16;
  const u32x bg = (u32x)(al[((bb[2] >> 0) & 255)]) << 0 | (u32x)(al[((bb[2] >> 8) & 255)]) << 16;
  const u32x bh = (u32x)(al[((bb[2] >> 16) & 255)]) << 0 | (u32x)(al[((bb[2] >> 24) & 255)]) << 16;
  const u32x bi = (u32x)(al[((bb[3] >> 0) & 255)]) << 0 | (u32x)(al[((bb[3] >> 8) & 255)]) << 16;

  const u32 bj = 0x0e0e0e0e;
  ...
  u32x bk = bb[0] & 0xffffffff;
  u32x bl = bb[1] & 0xffffffff;
  u32x bm = bb[2] & 0x000000ff;
  u32x bn = bb[3] & 0x00000000;

  {
    if (((bk) == at[0]) && ((bl) == at[1]) && ((bm) == at[2]) && ((bn) == at[3])) {
      const u32 bo = ad + 0;
      if (atomic_add(&q[bo], 1) == 0) {
        AA(o, t, y, 0, bo, ag, au);
      }
    }
  };
