...
void CL(u32 a[16], __local u32 (*b)[256], __local u32 (*c)[256]) {
  const u32 d[10 + 1] = {
  ...
  for (j = 1; j <= 10; j++) {
    ...
    for (m = 0; m < 8; m++) {
      const u32 n = f[(m + 8) & 7] >> 24;
      const u32 o = f[(m + 7) & 7] >> 16;
      const u32 p = f[(m + 6) & 7] >> 8;
      const u32 q = f[(m + 5) & 7] >> 0;
      const u32 r = g[(m + 4) & 7] >> 24;
      const u32 s = g[(m + 3) & 7] >> 16;
      const u32 t = g[(m + 2) & 7] >> 8;
      const u32 u = g[(m + 1) & 7] >> 0;

      k[m] = (b)[(0)][(n & 0xff)] ^ (b)[(1)][(o & 0xff)] ^ (b)[(2)][(p & 0xff)] ^ (b)[(3)][(q & 0xff)] ^ (b)[(4)][(r & 0xff)] ^ (b)[(5)][(s & 0xff)] ^ (b)[(6)][(t & 0xff)] ^ (b)[(7)][(u & 0xff)];

      l[m] = (c)[(0)][(n & 0xff)] ^ (c)[(1)][(o & 0xff)] ^ (c)[(2)][(p & 0xff)] ^ (c)[(3)][(q & 0xff)] ^ (c)[(4)][(r & 0xff)] ^ (c)[(5)][(s & 0xff)] ^ (c)[(6)][(t & 0xff)] ^ (c)[(7)][(u & 0xff)];
    }
    ...
    for (m = 0; m < 8; m++) {
      const u32 n = h[(m + 8) & 7] >> 24;
      const u32 o = h[(m + 7) & 7] >> 16;
      const u32 p = h[(m + 6) & 7] >> 8;
      const u32 q = h[(m + 5) & 7] >> 0;
      const u32 r = i[(m + 4) & 7] >> 24;
      const u32 s = i[(m + 3) & 7] >> 16;
      const u32 t = i[(m + 2) & 7] >> 8;
      const u32 u = i[(m + 1) & 7] >> 0;
...
void CM(const u32 a[16], u32 b[16], __local u32 (*c)[256], __local u32 (*d)[256]) {
  const u32 e[10 + 1] = {
      0x00000000, 0x1823c6e8, 0x36a6d2f5, 0x60bc9b8e, 0x1de0d7c2, 0x157737e5, 0x58c9290a, 0xbd5d10f4, 0xe427418b, 0xfbee7c66, 0xca2dbf07,
  };
  ...
  for (k = 1; k <= 10; k++) {
    ...
    for (n = 0; n < 8; n++) {
      const u32 o = g[(n + 8) & 7] >> 24;
      const u32 p = g[(n + 7) & 7] >> 16;
      const u32 q = g[(n + 6) & 7] >> 8;
      const u32 r = g[(n + 5) & 7] >> 0;
      const u32 s = h[(n + 4) & 7] >> 24;
      const u32 t = h[(n + 3) & 7] >> 16;
      const u32 u = h[(n + 2) & 7] >> 8;
      const u32 v = h[(n + 1) & 7] >> 0;
    ...
    for (n = 0; n < 8; n++) {
      const u32 o = i[(n + 8) & 7] >> 24;
      const u32 p = i[(n + 7) & 7] >> 16;
      const u32 q = i[(n + 6) & 7] >> 8;
      const u32 r = i[(n + 5) & 7] >> 0;
      const u32 s = j[(n + 4) & 7] >> 24;
      const u32 t = j[(n + 3) & 7] >> 16;
      const u32 u = j[(n + 2) & 7] >> 8;
      const u32 v = j[(n + 1) & 7] >> 0;
...
void CN(const u32 a[16], const u32 b[16], const u32 c[16], const u32 d[16], u32 e[16], __local u32 (*f)[256], __local u32 (*g)[256]) {
  e[0] = c[0];
  e[1] = c[1];
  e[2] = c[2];
  e[3] = c[3];
...
void CO(const u32 a[16], const u32 b[16], const u32 c[16], u32 d[16], __local u32 (*e)[256], __local u32 (*f)[256]) {
  d[0] = b[0];
  d[1] = b[1];
  d[2] = b[2];
  d[3] = b[3];
...
void CP(u32 a[16], u32 b[16], u32 c[16], __local u32 (*d)[256], __local u32 (*e)[256]) {
  a[0] ^= 0x36363636;
  a[1] ^= 0x36363636;
  a[2] ^= 0x36363636;
  a[3] ^= 0x36363636;
...
u32 CQ(const u32 a, const u32 b) {
...
__kernel void CR(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global tc_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global tc_t* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  am[0] = CQ(am[0], s[y].keyfile_buf[0]);
  am[1] = CQ(am[1], s[y].keyfile_buf[1]);
  am[2] = CQ(am[2], s[y].keyfile_buf[2]);
  am[3] = CQ(am[3], s[y].keyfile_buf[3]);
  an[0] = CQ(an[0], s[y].keyfile_buf[4]);
  an[1] = CQ(an[1], s[y].keyfile_buf[5]);
  an[2] = CQ(an[2], s[y].keyfile_buf[6]);
  an[3] = CQ(an[3], s[y].keyfile_buf[7]);
  ao[0] = CQ(ao[0], s[y].keyfile_buf[8]);
  ao[1] = CQ(ao[1], s[y].keyfile_buf[9]);
  ao[2] = CQ(ao[2], s[y].keyfile_buf[10]);
  ao[3] = CQ(ao[3], s[y].keyfile_buf[11]);
  ap[0] = CQ(ap[0], s[y].keyfile_buf[12]);
  ap[1] = CQ(ap[1], s[y].keyfile_buf[13]);
  ap[2] = CQ(ap[2], s[y].keyfile_buf[14]);
  ap[3] = CQ(ap[3], s[y].keyfile_buf[15]);
...
__kernel void CT(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global tc_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global tc_t* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
                 const u32 ac, const u32 ad, const u32 ae, const u32 af) {
  const u32 ag = get_global_id(0);
  const u32 ah = get_local_id(0);
