...
void BX(const u32 a[4], const u32 b[4], const u32 c[4], const u32 d[4], u32 e[5]) {
  u32 f = e[0];
  u32 g = e[1];
  u32 h = e[2];
  u32 i = e[3];
...
__kernel void BY(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global axcrypt_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa,
  ...
  e[ag].KEK[0] = am[0] ^ r[y].salt_buf[0];
  e[ag].KEK[1] = am[1] ^ r[y].salt_buf[1];
  e[ag].KEK[2] = am[2] ^ r[y].salt_buf[2];
  e[ag].KEK[3] = am[3] ^ r[y].salt_buf[3];

  e[ag].lsb[0] = r[y].salt_buf[6];
  e[ag].lsb[1] = r[y].salt_buf[7];
  e[ag].lsb[2] = r[y].salt_buf[8];
  e[ag].lsb[3] = r[y].salt_buf[9];

  e[ag].cipher[0] = r[y].salt_buf[4];
  e[ag].cipher[1] = r[y].salt_buf[5];
  e[ag].cipher[2] = 0;
  e[ag].cipher[3] = 0;
}

__kernel void BZ(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global axcrypt_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa,
                 const u32 ab, const u32 ac, const u32 ad, const u32 ae, const u32 af) {
  const u32 ag = get_global_id(0);
  ...
  ay[2] = e[ag].cipher[2];
  ay[3] = e[ag].cipher[3];
  u32 az = r[y].salt_iter - 1;

  for (u32 at = 0, ba = az - z; at < aa; at++, ba--) {
    ay[0] ^= M(2 * ba + 2);

    ay[2] = ax[2];
    ay[3] = ax[3];

    BW(ay, ay, av, aj, ak, al, am, an);
...
__kernel void CA(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global axcrypt_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa,
                 const u32 ab, const u32 ac, const u32 ad, const u32 ae, const u32 af) {
  const u32 ag = get_global_id(0);
  const u32 ah = get_local_id(0);
  ...
  if (e[ag].cipher[0] == 0xA6A6A6A6 && e[ag].cipher[1] == 0xA6A6A6A6) {
