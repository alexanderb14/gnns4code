...
__kernel void BX(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global void* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  u64x al = 0x736f6d6570736575;
  u64x am = 0x646f72616e646f6d;
  u64x an = 0x6c7967656e657261;
  u64x ao = 0x7465646279746573;

  al ^= F(r[y].salt_buf[1], r[y].salt_buf[0]);
  am ^= F(r[y].salt_buf[3], r[y].salt_buf[2]);
  an ^= F(r[y].salt_buf[1], r[y].salt_buf[0]);
  ao ^= F(r[y].salt_buf[3], r[y].salt_buf[2]);
  ...
  for (u32 ap = 0; ap < ab; ap += 1) {
    ...
    for (bi = 0, bj = 0; bi <= ar && bi < 16; bi += 8, bj += 2) {
      u64x bk = F(ba[bj + 1], ba[bj + 0]);
    ...
    for (bj = 0; bi <= ar && bi < 32; bi += 8, bj += 2) {
      u64x bk = F(bb[bj + 1], bb[bj + 0]);
    ...
    {
      const u32 bp[4] = {bmm, bnn, boo, boo};
      if (Z(bp, g, h, i, j, k, l, m, n, v, w, x)) {
        int bq = X(bp, ac, &p[ad]);
        if (bq != -1) {
          const u32 br = ad + bq;
          if (atomic_add(&q[br], 1) == 0) {
            AA(o, t, y, bq, br, ah, ap);
          }
        }
      }
    };
...
for (u32 aq = 0; aq < ab; aq += 1) {
  ...
  for (bj = 0, bk = 0; bj <= as && bj < 16; bj += 8, bk += 2) {
    u64x bl = F(bb[bk + 1], bb[bk + 0]);
  ...
  for (bk = 0; bj <= as && bj < 32; bj += 8, bk += 2) {
    u64x bl = F(bc[bk + 1], bc[bk + 0]);
  ...
  {
    if (((bn) == ap[0]) && ((bo) == ap[1]) && ((bp) == ap[2]) && ((bp) == ap[3])) {
      const u32 bq = ad + 0;
      if (atomic_add(&q[bq], 1) == 0) {
        AA(o, t, y, 0, bq, ah, aq);
      }
    }
  };
