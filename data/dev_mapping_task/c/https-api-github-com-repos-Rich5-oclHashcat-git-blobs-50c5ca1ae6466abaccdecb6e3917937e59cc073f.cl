...
void BU(u32* a, u32* b, __local u32* c, __local u32* d, __local u32* e, __local u32* f, __local u32* g) {
  ...
  for (u32 h = 0, i = 0; h < 10; h += 1, i += 4) {
    u32 j = b[i + 3];

    j = (e[(j >> 16) & 0xff] & 0xff000000) ^ (f[(j >> 8) & 0xff] & 0x00ff0000) ^ (c[(j >> 0) & 0xff] & 0x0000ff00) ^ (d[(j >> 24) & 0xff] & 0x000000ff);
...
void BW(const u32* a, u32* b, const u32* c, __local u32* d, __local u32* e, __local u32* f, __local u32* g, __local u32* h) {
  ...
  b[0] = (h[(m >> 24) & 0xff] & 0xff000000) ^ (h[(p >> 16) & 0xff] & 0x00ff0000) ^ (h[(o >> 8) & 0xff] & 0x0000ff00) ^ (h[(n >> 0) & 0xff] & 0x000000ff) ^ c[40];

  b[1] = (h[(n >> 24) & 0xff] & 0xff000000) ^ (h[(m >> 16) & 0xff] & 0x00ff0000) ^ (h[(p >> 8) & 0xff] & 0x0000ff00) ^ (h[(o >> 0) & 0xff] & 0x000000ff) ^ c[41];

  b[2] = (h[(o >> 24) & 0xff] & 0xff000000) ^ (h[(n >> 16) & 0xff] & 0x00ff0000) ^ (h[(m >> 8) & 0xff] & 0x0000ff00) ^ (h[(p >> 0) & 0xff] & 0x000000ff) ^ c[42];

  b[3] = (h[(p >> 24) & 0xff] & 0xff000000) ^ (h[(o >> 16) & 0xff] & 0x00ff0000) ^ (h[(n >> 8) & 0xff] & 0x0000ff00) ^ (h[(m >> 0) & 0xff] & 0x000000ff) ^ c[43];
}

void BX(u32* a, u32* b, __local u32* c, __local u32* d, __local u32* e, __local u32* f, __local u32* g) {
  b[0] = a[0];
  b[1] = a[1];
  b[2] = a[2];
  b[3] = a[3];
...
void BY(u32* a, __local u32* b, __local u32* c, __local u32* d, __local u32* e, __local u32* f, __local u32* g, __local u32* h, __local u32* i, __local u32* j, __local u32* k) {
...
void BZ(const u32* a, u32* b, const u32* c, __local u32* d, __local u32* e, __local u32* f, __local u32* g, __local u32* h) {
  u32 i = a[0] ^ c[0];
  u32 j = a[1] ^ c[1];
  u32 k = a[2] ^ c[2];
  u32 l = a[3] ^ c[3];
...
void CA(const u32* a, u32* b, const u32* c, __local u32* d, __local u32* e, __local u32* f, __local u32* g, __local u32* h) {
  u32 i = a[0] ^ c[0];
  u32 j = a[1] ^ c[1];
  u32 k = a[2] ^ c[2];
  u32 l = a[3] ^ c[3];
...
void CB(const u32 a[4], const u32 b[4], const u32 c[4], const u32 d[4], u32 e[8]) {
  u32 f = e[0];
  u32 g = e[1];
  u32 h = e[2];
  u32 i = e[3];
...
void CC(const u32 a[4], const u32 b[4], const u32 c[4], const u32 d[4], u32 e[5]) {
  u32 f = e[0];
  u32 g = e[1];
  u32 h = e[2];
  u32 i = e[3];
...
void CD(u32 a[4], u32 b[4], u32 c[4], u32 d[4], u32 e[5], u32 f[5]) {
  a[0] = a[0] ^ 0x36363636;
  a[1] = a[1] ^ 0x36363636;
  a[2] = a[2] ^ 0x36363636;
  a[3] = a[3] ^ 0x36363636;
...
void CE(u32 a[4], u32 b[4], u32 c[4], u32 d[4], u32 e[5], u32 f[5], u32 g[5]) {
  g[0] = e[0];
  g[1] = e[1];
  g[2] = e[2];
  g[3] = e[3];
...
__kernel void CF(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global androidfde_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global androidfde_t* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z,
  ...
  for (u32 ap = 0, aq = 1; ap < 8; ap += 5, aq += 1) {
    ah[0] = am[0];
    ah[1] = am[1];
    ah[2] = am[2];
    ah[3] = am[3];
...
__kernel void CG(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global androidfde_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global androidfde_t* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z,
                 const u32 aa, const u32 ab, const u32 ac, const u32 ad, const u32 ae, const u32 af) {
  const u32 ag = get_global_id(0);
...
__kernel void CH(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global androidfde_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global androidfde_t* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z,
                 const u32 aa, const u32 ab, const u32 ac, const u32 ad, const u32 ae, const u32 af) {
  const u32 ag = get_global_id(0);
  ...
  aw[0] = p[ad].digest_buf[0];
  aw[1] = p[ad].digest_buf[1];
  aw[2] = p[ad].digest_buf[2];
  aw[3] = p[ad].digest_buf[3];
  ...
  BU(az, au, ao, ap, aq, ar, as);

  for (u32 at = 0; at < 44; at++)
    av[at] = au[at];

  BV(av, aj, ak, al, am, an, ao, ap, aq, ar, as);

  BW(aw, ay, av, aj, ak, al, am, an);
...

  {
    ...
    BX(az, au, ao, ap, aq, ar, as);

    u32 bj[4];

    CA(aw, bj, au, ao, ap, aq, ar, as);
    ...
    if ((bk == 0x4f44534d) && (bl == 0x302e3553)) {
...

  {
    ...
    for (u32 at = 4; at < 16; at += 4) {
      aw[0] = s[y].data[256 + at + 0];
      aw[1] = s[y].data[256 + at + 1];
      aw[2] = s[y].data[256 + at + 2];
      aw[3] = s[y].data[256 + at + 3];

      ax[0] = s[y].data[256 + at + 0 - 4];
      ax[1] = s[y].data[256 + at + 1 - 4];
      ax[2] = s[y].data[256 + at + 2 - 4];
      ax[3] = s[y].data[256 + at + 3 - 4];
    ...
    if ((bo[5] < 2) && (bo[6] < 16) && ((bo[14] & 0xffff) == 0xEF53)) {
      AA(o, t, y, 0, ad + 0, ag, 0);
    }
  }
}