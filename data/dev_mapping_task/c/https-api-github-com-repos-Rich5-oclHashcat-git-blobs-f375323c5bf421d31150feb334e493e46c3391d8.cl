...
void CL(const u32 a[4], const u32 b[4], const u32 c[4], const u32 d[4], u32 e[8]) {
  u32 f = e[0];
  u32 g = e[1];
  u32 h = e[2];
  u32 i = e[3];
...
void CM(u32 a[4], u32 b[4], u32 c[4], u32 d[4], u32 e[8], u32 f[8]) {
  a[0] = a[0] ^ 0x36363636;
  a[1] = a[1] ^ 0x36363636;
  a[2] = a[2] ^ 0x36363636;
  a[3] = a[3] ^ 0x36363636;
...
void CN(u32 a[4], u32 b[4], u32 c[4], u32 d[4], u32 e[8], u32 f[8], u32 g[8]) {
  g[0] = e[0];
  g[1] = e[1];
  g[2] = e[2];
  g[3] = e[3];
...
void CO(u32 a[4], u32 b[4], u32 c[4], u32 d[4], u32 e[4], u32 f[4], u32 g[4], u32 h[4], u32 i[8], u32 j[8], u32 k[8]) {
  k[0] = i[0];
  k[1] = i[1];
  k[2] = i[2];
  k[3] = i[3];
...
__kernel void CQ(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global tc_tmp_t* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global tc_t* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  al[0] = M(s[y].salt_buf[0]);
  al[1] = M(s[y].salt_buf[1]);
  al[2] = M(s[y].salt_buf[2]);
  al[3] = M(s[y].salt_buf[3]);
  am[0] = M(s[y].salt_buf[4]);
  am[1] = M(s[y].salt_buf[5]);
  am[2] = M(s[y].salt_buf[6]);
  am[3] = M(s[y].salt_buf[7]);
  an[0] = M(s[y].salt_buf[8]);
  an[1] = M(s[y].salt_buf[9]);
  an[2] = M(s[y].salt_buf[10]);
  an[3] = M(s[y].salt_buf[11]);
  ao[0] = M(s[y].salt_buf[12]);
  ao[1] = M(s[y].salt_buf[13]);
  ao[2] = M(s[y].salt_buf[14]);
  ao[3] = M(s[y].salt_buf[15]);
  ap[0] = 0;
  ap[1] = 0x80000000;
  ap[2] = 0;
  ap[3] = 0;
  ...
  const u32 at = r[0].truecrypt_mdlen;

  u32 au[8];
  u32 av[8];

  CM(ah, ai, aj, ak, au, av);
  ...
  for (u32 aw = 0, ax = 1; aw < ((at / 8) / 4); aw += 8, ax += 1) {
    ap[0] = ax;

    u32 ay[8];

    CO(al, am, an, ao, ap, aq, ar, as, au, av, ay);
