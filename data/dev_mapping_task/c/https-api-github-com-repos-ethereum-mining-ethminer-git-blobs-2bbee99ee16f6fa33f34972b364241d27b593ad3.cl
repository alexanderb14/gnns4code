...
__kernel void
B(__global volatile search_results* restrict a, __constant hash32_t const* b, __global hash128_t const* c, ulong d, ulong e, uint f) {
  uint const g = get_global_id(0);
  uint const h = g & (2 - 1);
  uint const i = (g % 128) >> 1;

  __local hash64_t j[(128 / 2)];
  ...
  for (uint o = 0; o < 2; o++) {
    if (o == h) {
      k->uint16s[0] = ((uint16*)l)[0];
    }
    barrier(1);

    uint16 p = k->uint16s[0];
    ...
    for (uint r = 0; r < 64; r += 16) {
      bool s = h == ((r >> 4) & (2 - 1));

      for (uint m = 0; m != 16; ++m) {
        if (s) {
          k->uints[0] = ((q ^ (r + m)) * 0x01000193 ^ (((uint*)&p)[m])) % 8388593;
        }
        barrier(1);

        for (uint m = 0; m < 16; m++)
          ((uint*)&p)[m] = ((((uint*)&p)[m]) * 0x01000193 ^ (c[k->uints[0]].uints[16 * h + m]));

        barrier(1);
      }
    }

    k->uint4s[h] = (uint4)(((((((p0123.x) * 0x01000193 ^ (p0123.y))) * 0x01000193 ^ (p0123.z))) * 0x01000193 ^ (p0123.w)), ((((((p4567.x) * 0x01000193 ^ (p4567.y))) * 0x01000193 ^ (p4567.z))) * 0x01000193 ^ (p4567.w)), ((((((p89ab.x) * 0x01000193 ^ (p89ab.y))) * 0x01000193 ^ (p89ab.z))) * 0x01000193 ^ (p89ab.w)), ((((((pcdef.x) * 0x01000193 ^ (pcdef.y))) * 0x01000193 ^ (pcdef.z))) * 0x01000193 ^ (pcdef.w)));

    barrier(1);

    if (o == h) {
      ((ulong4*)l)[2] = k->ulong4s[0];
    }
    barrier(1);
  }
