...
void G(const uint a, uint4 b[8], __global uint4* restrict c) {
  D(b);
  const uint d = 8;
  const uint e = (1024 / 16 + (1024 % 16 > 0));
  const uint f = 128;
  const uint g = a % f;

  for (uint h = 0; h < 1024 / 16; ++h) {
    for (uint i = 0; i < d; ++i)
      c[i + g * (d) + h * (f) * (d)] = b[i];
    for (uint j = 0; j < 16; ++j)
      F(b);
...
__attribute__((reqd_work_group_size(128, 1, 1))) __kernel void H(

    const uint a,

    __global const uint4* restrict b, volatile __global uint* restrict c, __global uint4* restrict d, const uint4 e, const uint4 f, const uint g) {
  ...
  uint4 i[8];
  uint4 j, k, l, m, n, o;
 uint4 p = (uint4)(bx,by,bz,h
 uint4 q = e, r = f;

 A(&q,&r, p, (uint4)(Gbb[84],0,0,0), (uint4)(0,0,0,0), (uint4)(0,0,0, Gbb[86]));
 B(&l,&m, q^ Gb[82], r^ Gb[82], Gb[82], Gb[82]);
 B(&j,&k, q^ Gb[83], r^ Gb[83], Gb[83], Gb[83]);

 n = j;
 o = k;
 A(&j, &k, b[0],b[1],b[2],b[3]);
 ...
 for (uint s=0; s<4; s++)
 {
    ...
    A(&q, &r, p, (uint4)(s + 1, Gbb[84], 0, 0), (uint4)(0, 0, 0, 0), (uint4)(0, 0, 0, Gbb[87]));
    A(i + s * 2, i + s * 2 + 1, q, r, (uint4)(Gbb[84], 0U, 0U, 0U), (uint4)(0U, 0U, 0U, Gbb[88]));
 }
 G(h, i, d);
 A(&n,&o, i[0], i[1], i[2], i[3]);
 A(&n,&o, i[4], i[5], i[6], i[7]);
 C(&n,&o);
 A(&l,&m, n, o, (uint4)(Gbb[84], 0U, 0U, 0U), (uint4)(0U, 0U, 0U, Gbb[88]));

 bool t = ((rotate(m.w & Ga[0],24U)|rotate(m.w & Ga[1],8U)) <= g);
 if (t)
  c[c[(0xFF)]++] = h;
}