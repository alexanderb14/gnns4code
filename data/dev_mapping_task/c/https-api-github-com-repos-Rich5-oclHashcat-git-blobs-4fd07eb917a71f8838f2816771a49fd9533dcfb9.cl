...
__kernel void BZ(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global void* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  for (u32 an = 0; an < ab; an += 1) {
    ...
    for (bl = 0; bl < bj - 4; bl += 4) {
      const u32 bn = U((Ga)[((bk + bl) / 4) + 1], (Ga)[((bk + bl) / 4) + 0], (bk + bl));

      {
        const u32 bo = ((bm + bl) & 3) * 8;
        const u64 bp = (u64)(bn) << bo;
        (bh)[((bm + bl) / 4) + 0] |= bp;
        (bh)[((bm + bl) / 4) + 1] = bp >> 32;
      };
    }

    const u32 bq = 0xffffffff >> (((bl - bj) & 3) * 8);

    const u32 bn = U((Ga)[((bk + bl) / 4) + 1], (Ga)[((bk + bl) / 4) + 0], (bk + bl)) & bq;
    ...
    for (bl = 0; bl < am + 1; bl += 4) {
      const u32 bn = al[bl / 4];
    ...
    for (br = bm, bs = 0; br >= 56; br -= 64, bs += 16) {
      BX(&bh[bs]);

      BY(&bh[bs + 0], &bh[bs + 4], &bh[bs + 8], &bh[bs + 12], bi);
    }
    ...
    {
      const u32 bt[4] = {bi bi bi bi};
      if (Z(bt, g, h, i, j, k, l, m, n, v, w, x)) {
        int bu = X(bt, ac, &p[ad]);
        if (bu != -1) {
          const u32 bv = ad + bu;
          if (atomic_add(&q[bv], 1) == 0) {
            AA(o, t, y, bu, bv, ah, an);
          }
        }
      }
    };
...
for (u32 ao = 0; ao < ab; ao += 1) {
  ...
  for (bm = 0; bm < bk - 4; bm += 4) {
    const u32 bo = U((Ga)[((bl + bm) / 4) + 1], (Ga)[((bl + bm) / 4) + 0], (bl + bm));

    {
      const u32 bp = ((bn + bm) & 3) * 8;
      const u64 bq = (u64)(bo) << bp;
      (bi)[((bn + bm) / 4) + 0] |= bq;
      (bi)[((bn + bm) / 4) + 1] = bq >> 32;
    };
  }

  const u32 br = 0xffffffff >> (((bm - bk) & 3) * 8);

  const u32 bo = U((Ga)[((bl + bm) / 4) + 1], (Ga)[((bl + bm) / 4) + 0], (bl + bm)) & br;
  ...
  for (bm = 0; bm < am + 1; bm += 4) {
    const u32 bo = al[bm / 4];
  ...
  for (bs = bn, bt = 0; bs >= 56; bs -= 64, bt += 16) {
    BX(&bi[bt]);

    BY(&bi[bt + 0], &bi[bt + 4], &bi[bt + 8], &bi[bt + 12], bj);
  }
  ...
  {
    if (((bj[3]) == an[0]) && ((bj[4]) == an[1]) && ((bj[2]) == an[2]) && ((bj[1]) == an[3])) {
      const u32 bu = ad + 0;
      if (atomic_add(&q[bu], 1) == 0) {
        AA(o, t, y, 0, bu, ah, ao);
      }
    }
  };
