...
void CB(__local RC4_KEY* a, u32 b[4], u32 c[4], u32 d[4], u32 e[4], const u32 f, __global pw_t* g, __global kernel_rule_t* h, __global comb_t* i, __global bf_t* j, __global void* k, __global void* l, __global u32* m, __global u32* n, __global u32* o, __global u32* p, __global u32* q, __global u32* r, __global u32* s, __global u32* t, __global plain_t* u, __global digest_t* v, __global u32* w, __global salt_t* x, __global oldoffice01_t* y, __global u32* z, __global u32* aa, const u32 ab,
  ...
  for (u32 aq = 0; aq < ah; aq += 1) {
    ...
    { const u32 ba[4] = { ay0], ay1], ay2], ay3] };
      if (Z(ba, m, n, o, p, q, r, s, t, ab, ac, ad)) {
        int bb = X(ba, ai, &v[aj]);
        if (bb != -1) {
          const u32 bc = aj + bb;
          if (atomic_add(&w[bc], 1) == 0) {
            AA(u, z, ae, bb, bc, ak, aq);
          }
        }
      }
    };
...
for (u32 ar = 0; ar < ah; ar += 1) {
  ...
  {
    if (((az[0]) == ap[0]) && ((az[1]) == ap[1]) && ((az[2]) == ap[2]) && ((az[3]) == ap[3])) {
      const u32 bb = aj + 0;
      if (atomic_add(&w[bb], 1) == 0) {
        AA(u, z, ae, 0, bb, ak, ar);
      }
    }
  };
}
