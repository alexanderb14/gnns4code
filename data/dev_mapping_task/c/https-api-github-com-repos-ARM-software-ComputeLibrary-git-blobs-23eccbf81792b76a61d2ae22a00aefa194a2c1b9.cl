...
__kernel void J(__global uchar* a, uint b, uint c, uint d, uint e, uint f, uint g, uint h, __global uchar* i, uint j, uint k, uint l, uint m, uint n, uint o, uint p, int2 q, int2 r) {
  Tensor3D s = D(a, h, b, c, d, e, f, g);
  Tensor3D t = D(i, p, j, 0, l, 0, n, 0);

  int3 u = (int3)(get_global_id(0), get_global_id(1), get_global_id(2));

  int v = u.x + u.y * q.x + u.z * q.x * q.y;
  ...
  *((__global DATA_TYPE*)H(&t, w.x, w.y, w.z)) = *((__global DATA_TYPE*)s.ptr);
}