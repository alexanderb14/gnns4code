...
__kernel void DY(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global void* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  for (u32 al = 0; al < ab; al += 1) {
    ...
    const u32x aq = BW(ai, aj, ak, b, al, am, an);
    ...
    for (ax = 0, ay = 0; ax <= (int)aq - 4; ax += 4, ay += 1) {
      const u32x az = ar[ay];

      {
        as ^= (((as & 0x3f) + aw) * ((az >> 0) & 0xff)) + (as << 8);
        at += (at << 8) ^ as;
        aw += (az >> 0) & 0xff;
      };
      {
        as ^= (((as & 0x3f) + aw) * ((az >> 8) & 0xff)) + (as << 8);
        at += (at << 8) ^ as;
        aw += (az >> 8) & 0xff;
      };
      {
        as ^= (((as & 0x3f) + aw) * ((az >> 16) & 0xff)) + (as << 8);
        at += (at << 8) ^ as;
        aw += (az >> 16) & 0xff;
      };
      {
        as ^= (((as & 0x3f) + aw) * ((az >> 24) & 0xff)) + (as << 8);
        at += (at << 8) ^ as;
        aw += (az >> 24) & 0xff;
      };
    }
...
for (u32 am = 0; am < ab; am += 1) {
  ...
  for (ay = 0, az = 0; ay <= (int)ar - 4; ay += 4, az += 1) {
    const u32x ba = as[az];

    {
      at ^= (((at & 0x3f) + ax) * ((ba >> 0) & 0xff)) + (at << 8);
      au += (au << 8) ^ at;
      ax += (ba >> 0) & 0xff;
    };
    {
      at ^= (((at & 0x3f) + ax) * ((ba >> 8) & 0xff)) + (at << 8);
      au += (au << 8) ^ at;
      ax += (ba >> 8) & 0xff;
    };
    {
      at ^= (((at & 0x3f) + ax) * ((ba >> 16) & 0xff)) + (at << 8);
      au += (au << 8) ^ at;
      ax += (ba >> 16) & 0xff;
    };
    {
      at ^= (((at & 0x3f) + ax) * ((ba >> 24) & 0xff)) + (at << 8);
      au += (au << 8) ^ at;
      ax += (ba >> 24) & 0xff;
    };
  }
  ...
  {
    if (((at) == al[0]) && ((au) == al[1]) && ((bc) == al[2]) && ((bc) == al[3])) {
      const u32 bd = ad + 0;
