...
__kernel void F(int a, __global float* b, __global float* c, __global float* d, float e, float f, float g, float h, int i) {
  ...
  b[j] = b[j] + (g * sqrt(1.0 - __clc_pow(f, i)) / (1.0 - __clc_pow(e, i)) * c[j] / (sqrt(d[j]) + h));
}
...
__kernel void W(int a, __global float* b, float c, __global float* d) {
  int e = get_global_id(2) * get_global_size(0) * get_global_size(1) + get_global_id(1) * get_global_size(0) + get_global_id(0);
  if (e < a && d[e] == c)
    b[e] = c;
}
__kernel void X(int a, __global float* b, int c, int d, __global float* e, int f, int g) {
  int h = get_global_id(2) * get_global_size(0) * get_global_size(1) + get_global_id(1) * get_global_size(0) + get_global_id(0);
  if (h < a)
    e[h * g + f] = b[h * d + c];
}
...
__kernel void Z(__global float* a, int b, int c, int d, __global float* e) {
  __local float f[512];
...
__kernel void AA(__global float* a, __global float* b, int c, int d, int e, __global float* f) {
  __local float g[512];
  int h = get_global_id(1);
  int i = get_global_id(0);
  float j = 0;
...
__kernel void AB(int a, __global float* b, int c, int d, int e, int f, __global float* g) {
  int h = get_global_id(2) * get_global_size(0) * get_global_size(1) + get_global_id(1) * get_global_size(0) + get_global_id(0);
  if (h >= a)
    return;
  int i = h % c;
...
__kernel void AC(int a, __global float* b, float c, __global float* d, float e) {
  int f = get_global_id(2) * get_global_size(0) * get_global_size(1) + get_global_id(1) * get_global_size(0) + get_global_id(0);
  if (f < a && d[f] == c)
    b[f] *= e;
}
__kernel void AD(int a, int b, int c, int d, int e, int f, int g, int h, int i, int j, __global float* k, int l, int m, int n, __global float* o) {
  int p = get_global_id(2) * get_global_size(0) * get_global_size(1) + get_global_id(1) * get_global_size(0) + get_global_id(0);
  if (p >= a)
    return;
  int q = p % b;
  p /= b;
  ...
  p /= d;
  int t = p % g;
  int u = q * f + l * (r * f + m * (s + n * t));
  int v = q * e + h * (r * e + i * (s + j * t));
  o[u] += k[v];
}
