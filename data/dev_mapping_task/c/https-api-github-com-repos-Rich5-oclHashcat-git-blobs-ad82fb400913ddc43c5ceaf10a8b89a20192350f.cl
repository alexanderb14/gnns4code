...
__kernel void DY(__global pw_t* a, __global kernel_rule_t* b, __global comb_t* c, __global bf_t* d, __global void* e, __global void* f, __global u32* g, __global u32* h, __global u32* i, __global u32* j, __global u32* k, __global u32* l, __global u32* m, __global u32* n, __global plain_t* o, __global digest_t* p, __global u32* q, __global salt_t* r, __global void* s, __global u32* t, __global u32* u, const u32 v, const u32 w, const u32 x, const u32 y, const u32 z, const u32 aa, const u32 ab,
  ...
  for (u32 ap = 0; ap < ab; ap += 1) {
    ...
    const u32x au = BW(ai, aj, ak, b, ap, aq, ar);
    ...
    for (az = 0, ba = 0; az <= au && az < 16; az += 8, ba += 2) {
      u64x bb = F(aq[ba + 1], aq[ba + 0]);
    ...
    for (ba = 0; az <= au && az < 32; az += 8, ba += 2) {
      u64x bb = F(ar[ba + 1], ar[ba + 0]);
...
for (u32 aq = 0; aq < ab; aq += 1) {
  ...
  for (ba = 0, bb = 0; ba <= av && ba < 16; ba += 8, bb += 2) {
    u64x bc = F(ar[bb + 1], ar[bb + 0]);
  ...
  for (bb = 0; ba <= av && ba < 32; ba += 8, bb += 2) {
    u64x bc = F(as[bb + 1], as[bb + 0]);
  ...
  {
    if (((be) == ap[0]) && ((bf) == ap[1]) && ((bg) == ap[2]) && ((bg) == ap[3])) {
      const u32 bh = ad + 0;
