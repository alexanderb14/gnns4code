...
B(__global volatile uint* restrict a, __constant hash32_t const* b, __global hash128_t const* c, ulong d, ulong e, uint f) {
  ...
  uint const h = g & (8 - 1);
  uint const i = (g % 128) >> 3;

  __local hash64_t j[(128 / 8)];
  ...
  for (uint o = 0; o < 8; o++) {
    ...
    for (uint r = 0; r < 64; r += 4) {
      ...
      for (uint m = 0; m != 4; ++m) {
        ...
        barrier(1);
        p = ((p)*0x01000193 ^ (c[k->uints[0]].uint4s[h]));

        barrier(1);
      }
    }
    k->uints[h] = ((((((p.x) * 0x01000193 ^ (p.y))) * 0x01000193 ^ (p.z))) * 0x01000193 ^ (p.w));
  ...
  if (__builtin_astype(__builtin_astype(l[0], uchar8).s76543210, ulong) < e) {
    uint t = min(63U, atomic_add(&a[0], 1) + 1);
    a[t] = g;
  }
}
