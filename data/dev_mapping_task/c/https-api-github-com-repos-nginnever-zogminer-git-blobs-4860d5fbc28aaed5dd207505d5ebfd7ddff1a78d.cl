...
uint D(uint a, __global char* b, uint c, uint d, uint e, __global ulong* f, __global ulong* g) {
  ulong h, i, j;
...
void E(uint a, __global char* b, __global char* c, __global uint* d) {
...
__kernel __attribute__((reqd_work_group_size(64, 1, 1))) void F(__global char* a, __global char* b, __global uint* c) {
  E(1, a, b, c);
}
__kernel __attribute__((reqd_work_group_size(64, 1, 1))) void G(__global char* a, __global char* b, __global uint* c) {
  E(2, a, b, c);
}
...
__kernel __attribute__((reqd_work_group_size(64, 1, 1))) void M(__global char* a, __global char* b, __global uint* c, __global sols_t* d) {
  uint e = get_global_id(0);
  E(8, a, b, c);
  if (!e)
...
uint N(__global char* a, uint b, uint c, uint d) {
  return *(__global uint*)(a + c * ((1 << (((200 / (9 + 1)) + 1) - 20)) * 9) * 32 + d * 32 + b - 4);
}

void O(__global uint* a, uint b, __global char** c, uint d) {
  __global char* e = c[d % 2];
  uint f = b - 1;
  uint g = b * 2 - 1;
  uint h = (8 + ((d) / 2) * 4);
  do {
    a[g] = N(e, h, (a[f] >> 12), ((a[f] >> 6) & 0x3f));
    a[g - 1] = N(e, h, (a[f] >> 12), (a[f] & 0x3f));
    if (!f)
      break;
    f--;
    g -= 2;
...
void P(__global char** a, __global sols_t* b, uint c, uint d) {
  uint e;
  uint f;
  e = atomic_add(&b->nr, 1);
  if (e >= 2000)
  ...
  do {
    g--;
    O(&(b->values[e][0]), f, a, g);
    f *= 2;
  } while (g > 0);
  b->valid[e] = 1;
}

__kernel void Q(__global char* a, __global char* b, __global sols_t* c) {
  uint d = get_global_id(0);
