void A(__private uint4* a, int b); void B(__private uint4* a); void C(__private uint4* a); void D(__private uint4* a, int b, int c, const __constant uint4* d, int e); __kernel void E(__global uint4* a, const __global uint4* b) {
  __private uint4 c[32];
...
void C(__private uint4* a) {
  for (uint b = 8; b < 16; b++)
    a[b] = a[b].s1230;
  for (uint b = 16; b < 24; b++)
    a[b] = a[b].s2301;
  for (uint b = 24; b < 32; b++)
    a[b] = a[b].s3012;
}