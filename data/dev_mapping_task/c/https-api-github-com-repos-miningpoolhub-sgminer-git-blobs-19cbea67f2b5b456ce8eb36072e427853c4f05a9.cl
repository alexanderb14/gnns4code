...
__constant uint8 Gb = {0x00000000, 0x00000000, 0x80000000, 0x00000000, 0x00000000, 0x00000000, 0x00000000, 0x00000540};
...
__attribute__((reqd_work_group_size(128, 1, 1))) __kernel void B(__global const uchar* restrict a, __global uint* restrict b, const ulong c, uint8 d) {
  ...
  f.lo = ((__global const uint8*)a)[4];
  f.hi = Gb;
  f.hi.s0 = ((__global const uint*)a)[40];
  f.hi.s1 = ((__global const uint*)a)[41];
  f.s3 = e;
  g = A(f, d);
  f.lo = g;
  f.hi = Gc;
  ...
  if (__builtin_astype(__builtin_astype(g.s67, uchar8).s32107654, ulong) <= c)
    b[atomic_add(b + 0xFF, 1)] = e;
}